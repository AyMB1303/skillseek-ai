      ...
      {etat === "ok" && stats && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            <Carte libelle="Comptes actifs" valeur={stats.comptes.total} accent="accent" />
            <Carte libelle="Offres publiées" valeur={stats.offres.total} detail={`${stats.offres.ouvertes} ouverte(s)`} accent="cyan" />
            <Carte libelle="Candidatures reçues" valeur={stats.candidatures.total} accent="succes" />
            <Carte
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <section className="carte p-5">
              ...
              <div className="space-y-3">
                {Object.entries(stats.comptes.par_role).map(([role, nombre]) => {
                  ...
                  return (
                    <div key={role}>
                      ...
                      <div className="h-2 bg-fond rounded-full overflow-hidden">
                        <div className="h-full bg-accent rounded-full transition-all" style={{ width: `${part}%` }} />
                      </div>
                    </div>
                  );
                })}
...
function Carte({ libelle, valeur, detail, lien, accent = "accent" }) {
  ...
  const contenu = (
    <>
      ...
      <p className={`text-3xl font-bold mt-2 ${couleurs[accent]}`}>{valeur}</p>
      {detail && <p className="text-[11px] text-txt2 mt-1">{detail}</p>}
    </>
  );

  return lien ? (
    <Link href={lien} className="carte p-4 hover:border-accent transition-colors">{contenu}</Link>
  ) : (
